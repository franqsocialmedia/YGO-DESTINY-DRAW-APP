...
// ── Winrate — registro de W/L por deck; datos en localStorage bajo yugioh_winrates ──
const Winrate = {
    _KEY:        'yugioh_winrates',
    _KEY_SOLO:   'pz_winrate_standalone',
    ...
    getRecord: function(deckName) {
        if (!deckName) return this._getStandaloneRecord();
        const all = this.getAllRecords();
        return all[deckName] || { wins1st:0, losses1st:0, wins2nd:0, losses2nd:0 };
    },
    ...
    // going: '1st'|'2nd'   type: 'wins'|'losses'   delta: number
    change: function(going, type, delta) {
        const deckName = window.Deck?.name;
        const rec = this.getRecord(deckName);
        const key = type + (going === '1st' ? '1st' : '2nd');
        rec[key] = Math.max(0, (rec[key] || 0) + delta);
        if (deckName) this.saveRecord(deckName, rec);
        else this._saveStandaloneRecord(rec);
        this.refreshSection();
    },
    ...
    renderSection: function() {
        const deckName = window.Deck?.name;
        const rec = this.getRecord(deckName);
        const w1 = rec.wins1st   || 0, l1 = rec.losses1st || 0;
        const w2 = rec.wins2nd   || 0, l2 = rec.losses2nd || 0;
        const wr1 = this.calcWinrate(w1, l1);
        const wr2 = this.calcWinrate(w2, w2 + l2 - w2);
        const wr1real = this.calcWinrate(w1, l1);
        const wr2real = this.calcWinrate(w2, l2);
        const wrTotal = this.calcWinrate(w1 + w2, l1 + l2);
        const label = (wr) => wr === null ? '—' : `${wr}%`;
        const title = deckName ? `📊 Winrate: ${deckName}` : '📊 Winrate (Standalone)';
        return `
        <div class="wr-wrap">
            <h4 class="wr-title">${title}</h4>
            <div class="wr-grid">
                <div class="wr-col">
                    <div class="wr-col-title">Going 1st</div>
                    <div class="wr-score">${label(wr1real)}</div>
                    <div class="wr-counts">${w1}V / ${l1}D</div>
                    <div class="wr-btns">
                        <button class="wr-btn wr-win"  onclick="Winrate.change('1st','wins',1)">+W</button>
                        <button class="wr-btn wr-loss" onclick="Winrate.change('1st','losses',1)">+D</button>
                        <button class="wr-btn wr-undo" onclick="Winrate.change('1st','wins',-1)">−W</button>
                        <button class="wr-btn wr-undo" onclick="Winrate.change('1st','losses',-1)">−D</button>
                    </div>
                </div>
                <div class="wr-col">
                    <div class="wr-col-title">Going 2nd</div>
                    <div class="wr-score">${label(wr2real)}</div>
                    <div class="wr-counts">${w2}V / ${l2}D</div>
                    <div class="wr-btns">
                        <button class="wr-btn wr-win"  onclick="Winrate.change('2nd','wins',1)">+W</button>
                        <button class="wr-btn wr-loss" onclick="Winrate.change('2nd','losses',1)">+D</button>
                        <button class="wr-btn wr-undo" onclick="Winrate.change('2nd','wins',-1)">−W</button>
                        <button class="wr-btn wr-undo" onclick="Winrate.change('2nd','losses',-1)">−D</button>
                    </div>
                </div>
                <div class="wr-col wr-col-total">
                    <div class="wr-col-title">Total</div>
                    <div class="wr-score wr-score-total">${label(wrTotal)}</div>
                    <div class="wr-counts">${w1+w2}V / ${l1+l2}D</div>
                    <button class="wr-btn wr-reset" onclick="Winrate.resetDeck()">↺ Resetear</button>
                </div>
            </div>
        </div>`;
    },
    ...
    _getStandaloneRecord: function() {
        try { return JSON.parse(localStorage.getItem(this._KEY_SOLO)) || { wins1st:0, losses1st:0, wins2nd:0, losses2nd:0 }; }
        catch(_) { return { wins1st:0, losses1st:0, wins2nd:0, losses2nd:0 }; }
    },

    _saveStandaloneRecord: function(r) {
        localStorage.setItem(this._KEY_SOLO, JSON.stringify(r));
    },

    changeStandalone: function(going, type, delta) {
        const rec = this._getStandaloneRecord();
        const key = type + (going === '1st' ? '1st' : '2nd');
        rec[key] = Math.max(0, (rec[key] || 0) + delta);
        this._saveStandaloneRecord(rec);
        this.refreshSection();
    },
